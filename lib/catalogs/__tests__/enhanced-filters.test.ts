/**
 * Unit tests for Enhanced Filters (NINA-style)
 */
...
function createMockDSO(overrides: Partial<FilteredDSOResult> = {}): FilteredDSOResult {
  return {
    id: 'M31',
...
describe('DEFAULT_ENHANCED_FILTERS', () => {
  it('should have reasonable default values', () => {
    expect(DEFAULT_ENHANCED_FILTERS.objectName).toBe('');
    expect(DEFAULT_ENHANCED_FILTERS.minimumAltitude).toBe(0);
    expect(DEFAULT_ENHANCED_FILTERS.altitudeDuration).toBe(1);
    expect(DEFAULT_ENHANCED_FILTERS.minimumMoonDistance).toBe(0);
    expect(DEFAULT_ENHANCED_FILTERS.transitTimeFrom).toBeNull();
    expect(DEFAULT_ENHANCED_FILTERS.transitTimeThrough).toBeNull();
  });
});
...
describe('applyEnhancedFilters', () => {
  ...
  const mockObjects: FilteredDSOResult[] = [
  ...
  describe('name filter', () => {
    it('should filter by object name', () => {
      ...
      const result = applyEnhancedFilters(mockObjects, filters, latitude, longitude, referenceDate);
  ...
  describe('moon distance filter', () => {
    it('should filter by minimum moon distance', () => {
      ...
      const result = applyEnhancedFilters(objects, filters, latitude, longitude, referenceDate);
