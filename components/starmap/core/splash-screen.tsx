...
export function SplashScreen({ 
  ...
  return (
    <div 
      className={cn(
        'fixed inset-0 z-100 bg-black flex flex-col items-center justify-center overflow-hidden safe-area-inset',
        phase === 'fadeout' ? 'animate-fade-out pointer-events-none' : 'animate-fade-in'
      )}
    >
      ...
      >
        ...
        <p className="text-xs sm:text-sm text-slate-400 mb-6 sm:mb-8 text-center">
          {t('splash.tagline')}
        </p>
        ...
        <div 
          className={cn(
            'w-40 sm:w-48 transition-opacity duration-300',
      ...
      <div 
        className={cn(
          'absolute bottom-6 sm:bottom-8 text-center transition-opacity duration-500 px-4 safe-area-bottom',
          phase !== 'stars' ? 'opacity-100' : 'opacity-0'
        )}
      >
