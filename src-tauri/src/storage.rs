...
/// Delete store data
#[tauri::command]
pub async fn delete_store_data(
    app: AppHandle,
    store_name: String,
...
pub async fn export_all_data(
                ...
                if let Some(ext) = path.extension() {
                    if ext == "json" {
                        if let Some(name) = path.file_stem() {
                            ...
                            if !stores.contains_key(&name_str) {
                                let data = fs::read_to_string(&path)?;
                                let value: serde_json::Value = serde_json::from_str(&data)?;
                                stores.insert(name_str, value);
                            }
                        }
                    }
                }
    ...
    log::info!("Exported {} stores to {}", export_data.metadata.store_count, export_path);
